...
object Examples {
    ...
    fun printHelp() {
        println("Usage:")
        println("  ./gradlew :taokt-examples-native:runDebugExecutable<Target> --args=\"<example>\"")
        println("  e.g. ./gradlew :taokt-examples-native:runDebugExecutableLinuxX64 --args=\"window\"")
        println("Available examples:")
        ALL.keys.forEach { println("  - $it") }
    }

    private fun isWindows(): Boolean = Platform.hostOs == HostOs.Windows
    private fun isMac(): Boolean = Platform.hostOs == HostOs.Mac
    private fun isLinuxLike(): Boolean = Platform.hostOs == HostOs.Linux
    ...
    private class MultithreadedWorker(
        ...
        private val channel = Channel<TaoWindowEvent>(Channel.UNLIMITED)
                ...
                override fun handleEvent(event: TaoEvent, app: App): ControlFlow {
                    if (event is TaoEvent.NewEvents && event.cause == TaoStartCause.Init && workers.isEmpty()) {
                        repeat(windowCount) {
                            ...
                            val worker = MultithreadedWorker(w, baseWidth, baseHeight, backgroundScope)
                            workers[w.id()] = worker
                            worker.start()
                        }
                    }
                ...
                override fun handleEvent(event: TaoEvent, app: App): ControlFlow {
                    ...
                    when (event) {
                        is TaoEvent.NewEvents -> if (event.cause == TaoStartCause.Init && window == null) {
                            ...
                            window = w
                            redrawJob = backgroundScope.launch {
                                while (isActive) {
                                    delay(1_000)
                                    w.requestRedraw()
                                }
